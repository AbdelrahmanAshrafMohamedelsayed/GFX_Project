...
our::Texture2D *our::texture_utils::loadImage(const std::string &filename, bool generate_mipmap)
{
    ...
    //  - GL_TEXTURE_2D: the type of texture we are uploading
    //  - 0: We are setting the base image level (level 0)
    //  - GL_RGBA: The internal format of the image data (number of channels)
    //  - size.x, size.y: Width and height of the image
    //  - 0: border This value must be 0
    //  - GL_RGBA: Format of the image data
    //  - GL_UNSIGNED_BYTE: Type of the image data
    //  - pixels: Pointer to the image data pixels
    ...
    if (generate_mipmap)
    {
        // glGenerateMipmap generates all the mipmap levels for the specified texture object.
