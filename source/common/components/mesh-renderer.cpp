...
namespace our
{
    ...
    void MeshRendererComponent::deserialize(const nlohmann::json &data)
    {
        ...
        // Retrieves the names of the mesh and material assets from the input json object
        std::string meshName = data["mesh"].get<std::string>();
        std::string materialName = data["material"].get<std::string>();
        // Retrieves the mesh and material assets from the AssetLoader by passing their names
