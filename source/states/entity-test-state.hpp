...
class EntityTestState : public our::State
{
    ...
    void onDraw(double deltaTime) override
    {
        ...
        //  multiply them together to get the VP matrix
        ...
        for (auto &entity : world.getEntities())
        {
            ...
            //  First, we setup the material of the mesh renderer
            our::Material *material = meshRenderer->material;
            material->setup();
            // Then we set the transform uniform of the shader to the entity's local to world matrix multiplied by the view projection matrix
            material->shader->set("transform", VP * entity->getLocalToWorldMatrix());
            // Finally, we draw the mesh of the mesh renderer
